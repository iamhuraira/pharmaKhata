...
import { useResendTfaOtp, useVerifyTfaOtp } from '@/hooks/auth';

const VerifyTFAForm = () => {
  const { verifyTfaOtp, isLoading: isVerifying } = useVerifyTfaOtp();
  const { resendTfaOtp, isLoading } = useResendTfaOtp();
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!error && !isTouched) {
      verifyTfaOtp({ otp, email });
    }
  };

  useEffect(() => {
    const email = Cookies.get('TFAEmail');
    if (email) {
      setEmail(email);
    }
  }, []);
